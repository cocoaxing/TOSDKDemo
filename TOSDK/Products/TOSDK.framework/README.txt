...
SDK1.8.6
1. 支持Universal Link拉起w .x.以及返回App
2. SDK移除MTA库
...
受此影响，当你的应用在iOS 9中需要使用w .x.SDK的相关能力（分享、收藏、登录等）时，需要在“Info.plist”里增加如下代码：
...
支持未安装w .x.情况下Auth,具体见WXApi.h接口描述
3、w .x.开放平台新增了w .x.模块用户统计功能，便于开发者统计w .x.功能模块的用户使用和活跃情况。开发者需要在工程中链接上:SystemConfiguration.framework,libz.dylib,libsqlite3.0.dylib。
