...
@interface TOMJProperty()
@property (strong, nonatomic) NSMutableDictionary *propertyKeysDict;
@property (strong, nonatomic) NSMutableDictionary *objectClassInArrayDict;
@end

@implementation TOMJProperty
...
{
    TOMJProperty *propertyObj = objc_getAssociatedObject(self, property);
    if (propertyObj == nil) {
...
- (void)setProperty:(objc_property_t)property
{
    ...
    TOMJExtensionAssertParamNotNil(property);
    ...
    _type = [TOMJPropertyType cachedTypeWithCode:code];
}
...
- (NSArray *)propertyKeysWithStringKey:(NSString *)stringKey
{
    ...
    for (NSString *oldKey in oldKeys) {
        ...
        if (start != NSNotFound) { // 有索引的key
            ...
            if (prefixKey.length) {
                TOMJPropertyKey *propertyKey = [[TOMJPropertyKey alloc] init];
            ...
            for (NSInteger i = 0; i<cmps.count - 1; i++) {
                TOMJPropertyKey *subPropertyKey = [[TOMJPropertyKey alloc] init];
                subPropertyKey.type = TOMJPropertyKeyTypeArray;
