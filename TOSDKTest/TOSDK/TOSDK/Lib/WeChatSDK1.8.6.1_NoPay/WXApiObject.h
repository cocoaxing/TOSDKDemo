...
/** 由用户w .x.号和AppID组成的唯一标识，需要校验w .x.用户是否换号登录时填写*/
...
/*! @brief 第三方程序向w .x.终端请求认证的消息结构
 *
 * 第三方程序要向w .x.申请认证，并请求某些权限，需要调用WXApi的sendReq成员函数，
 * 向w .x.终端发送一个SendAuthReq消息结构。w .x.终端处理完后会向第三方程序发送一个处理结果。
 * @see SendAuthResp
 */
@interface SendAuthReq : BaseReq
/** 第三方程序要向w .x.申请认证，并请求某些权限，需要调用WXApi的sendReq成员函数，向w .x.终端发送一个SendAuthReq消息结构。w .x.终端处理完后会向第三方程序发送一个处理结果。
 * @see SendAuthResp
 * @note scope字符串长度不能超过1K
 */
@property (nonatomic, copy) NSString *scope;
/** 第三方程序本身用来标识其请求的唯一性，最后跳转回第三方程序时，由w .x.终端回传。
...
/*! @brief w .x.处理完第三方程序的认证和权限申请后向第三方程序回送的处理结果。
 *
 * 第三方程序要向w .x.申请认证，并请求某些权限，需要调用WXApi的sendReq成员函数，向w .x.终端发送一个SendAuthReq消息结构。
 * w .x.终端处理完后会向第三方程序发送一个SendAuthResp。
 * @see onResp
 */
@interface SendAuthResp : BaseResp
@property (nonatomic, copy, nullable) NSString *code;
/** 第三方程序发送时用来标识其请求的唯一性的标志，由第三方程序调用sendReq时传入，由w .x.终端回传
 * @note state字符串长度不能超过1K
 */
...
/*! @brief 第三方程序发送消息至w .x.终端程序的消息结构体
 *
 * 第三方程序向w .x.发送信息需要传入SendMessageToWXReq结构体，信息类型包括文本消息和多媒体消息，
 * 分别对应于text和message成员。调用该方法后，w .x.处理完信息会向第三方程序发送一个处理结果。
...
/*! @brief w .x.终端向第三方程序返回的SendMessageToWXReq处理结果。
 *
 * 第三方程序向w .x.终端发送SendMessageToWXReq后，w .x.发送回来的处理结果，该结果用SendMessageToWXResp表示。
 */
...
/*! @brief w .x.终端向第三方程序请求提供内容的消息结构体。
 *
 * w .x.终端向第三方程序请求提供内容，w .x.终端会向第三方程序发送GetMessageFromWXReq消息结构体，
...
/*! @brief w .x.终端向第三方程序请求提供内容，第三方程序向w .x.终端返回的消息结构体。
 *
 * w .x.终端向第三方程序请求提供内容，第三方程序调用sendResp向w .x.终端返回一个GetMessageFromWXResp消息结构体。
 */
 ...
 */
@property (nonatomic, strong) WXMediaMessage *message;
/** 向w .x.终端提供内容的消息类型，包括文本消息和多媒体消息两种，两者只能选择其一，不能同时发送文本和多媒体消息 */
@property (nonatomic, assign) BOOL bText;
@end
...
/*! @brief w .x.通知第三方程序，要求第三方程序显示的消息结构体。
 *
 * w .x.需要通知第三方程序显示或处理某些内容时，会向第三方程序发送ShowMessageFromWXReq消息结构体。
 * 第三方程序处理完内容后调用sendResp向w .x.终端发送ShowMessageFromWXResp。
 */
@interface ShowMessageFromWXReq : BaseReq
/** w .x.终端向第三方程序发送的要求第三方程序处理的多媒体内容
 * @see WXMediaMessage
 */
@property (nonatomic, strong) WXMediaMessage *message;
@property (nonatomic, copy) NSString *lang;
...
/*! @brief w .x.通知第三方程序，要求第三方程序显示或处理某些消息，第三方程序处理完后向w .x.终端发送的处理结果。
 *
 * w .x.需要通知第三方程序显示或处理某些内容时，会向第三方程序发送ShowMessageFromWXReq消息结构体。
 * 第三方程序处理完内容后调用sendResp向w .x.终端发送ShowMessageFromWXResp。
 */
...
/*! @brief w .x.终端打开第三方程序携带的消息结构体
 *
 *  w .x.向第三方发送的结构体，第三方不需要返回
 */
...
/* ! @brief 第三方通知w .x.启动内部浏览器，打开指定网页
 *
 *  第三方通知w .x.启动内部浏览器，打开指定Url对应的网页
 */
...
/*! @brief w .x.终端向第三方程序返回的OpenWebviewReq处理结果
 *
 * 第三方程序向w .x.终端发送OpenWebviewReq后，w .x.发送回来的处理结果，该结果用OpenWebviewResp表示
 */
...
/*! @brief 第三方通知w .x.启动内部浏览器，打开指定业务的网页
 *
 *
 */
...
/*! @brief w .x.终端向第三方程序返回的WXOpenBusinessWebViewResp处理结果。
 *
 * 第三方程序向w .x.终端发送WXOpenBusinessWebViewReq后，w .x.发送回来的处理结果，该结果用WXOpenBusinessWebViewResp表示。
 */
@interface WXOpenBusinessWebViewResp : BaseResp
/** 第三方程序自定义简单数据，w .x.终端会回传给第三方程序处理
 * @attention 长度不能超过2k
 */
...
/*! @brief w .x.终端向第三方程序返回的OpenRankListReq处理结果。
 *
 * 第三方程序向w .x.终端发送OpenRankListReq后，w .x.发送回来的处理结果，该结果用OpenRankListResp表示。
 */
 ...
 *
 * 第三方程序向w .x.终端发送包含WXAppExtendObject的多媒体消息，
 * w .x.需要处理该消息时，会调用该第三方程序来处理多媒体消息内容。
...
/** 若第三方程序不存在，w .x.终端会打开该url所指的App下载地址
 * @note 长度不能超过10K
 */
@property (nonatomic, copy) NSString *url;
/** 第三方程序自定义简单数据，w .x.终端会回传给第三方程序处理
...
/** App文件数据，该数据发送给w .x.好友，w .x.好友需要点击后下载数据，w .x.终端会回传给第三方程序处理
 * @note 大小不能超过10M
 */
 ...
 *
 * w .x.终端和第三方程序之间传递消息中包含的地理位置数据对象。
 * @see WXMediaMessage
 */
...
/*! @brief WXLaunchMiniProgramReq对象, 可实现通过sdk拉起w .x.小程序
 *
...
/*! @brief w .x.终端向第三方程序返回的WXLaunchMiniProgramReq处理结果。
 *
 * 第三方程序向w .x.终端发送WXLaunchMiniProgramReq后，w .x.发送回来的处理结果，该结果用WXLaunchMiniProgramResp表示。
 */
...
/*! @brief WXOpenBusinessViewReq对象, 可实现第三方通知w .x.启动，打开业务页面
 *
